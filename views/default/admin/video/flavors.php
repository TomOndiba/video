...
$flavors_delete = array();

if (empty($flavors)) {
	$flavors = elgg_echo('admin:video:no_flavors');
} else {
	foreach ($flavors as $key => $flavor) {
		// $flavors[$key][] = elgg_view('output/url', array(
		// TM:
		$flavors_delete = elgg_view('output/url', array(
			'href' => "action/video/delete_flavor?id=$key",
			'text' => elgg_echo('delete'),
			'confirm' => elgg_echo('flavor:delete:confirm'),
			
		));
	
		//TM: This is for admin to delete flavors
		 echo $flavors_delete;
	}
